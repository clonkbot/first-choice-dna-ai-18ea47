...
const menuItems = [
  ...
  { id: 'growth', label: 'Growth', icon: 'M13 10V3L4 14h7v7l9-11h-7z' },
];

const MobileNav: FC<MobileNavProps> = ({ activeSection, setActiveSection, isOpen, setIsOpen }) => {
  return (
    <>
      {/* Mobile Header */}
      <header className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-[#0d1d35]/95 backdrop-blur-lg border-b border-cyan-900/30 z-50 px-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-bold text-cyan-300">First Choice DNA</span>
        </div>
        <button
          onClick={() => setIsOpen(!isOpen)}
          className="w-10 h-10 flex items-center justify-center rounded-lg bg-cyan-900/30 text-cyan-400"
        >
      ...
      {/* Mobile Menu Overlay */}
      {isOpen && (
        <div className="lg:hidden fixed inset-0 z-40 bg-[#0a1628]/95 backdrop-blur-lg pt-20 px-4 pb-4 overflow-y-auto">
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-4 rounded-xl text-base font-medium transition-all ${
                  activeSection === item.id
                    ? 'bg-gradient-to-r from-cyan-500/20 to-teal-500/10 text-cyan-300'
